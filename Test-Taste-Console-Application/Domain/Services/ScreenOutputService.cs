        ...
        public void OutputAllPlanetsAndTheirAverageMoonGravityToConsole()
        {
            ...
            foreach(Planet planet in planets)
            {
                ...
                if(planet.HasMoons())
                {
                    ...
                    {
                        value = value+ Convert.ToDecimal(moon.gravity); // it will add all moons gravity
                    }
                    decimal avgvalue = value/ planet.Moons.Count;  // it will calculate the average 


                    ConsoleWriter.CreateText(new string[] { $"{planet.Id}", $"{avgvalue}" }, columnSizes);
                }
                else
                {
